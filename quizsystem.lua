...
local function showLeaderboard(leaderboardTab)
    ...
    if #sorted == 0 then
        ...
        for rank, entry in ipairs(sorted) do
            ...
            local line = place .. " - [Player Name] - " .. entry.points .. " pts"
            sayAsPlayer(line)
            task.wait(5) 
            pcall(function()
...
local function hookChat()
    Players.LocalPlayer.Chatted:Connect(function(msg)
        handleAnswer(msg, Players.LocalPlayer.Name, Players.LocalPlayer.DisplayName)
    end)
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Players.LocalPlayer then
            ...
            pl.Chatted:Connect(function(msg)
                handleAnswer(msg, pl.Name, pl.DisplayName)
            end)
        end
    end
    ...
    Players.PlayerAdded:Connect(function(p)
        p.Chatted:Connect(function(msg)
            handleAnswer(msg, p.Name, p.DisplayName)
        end)
    end)
end
...
for _, quiz in ipairs(Quizzes) do
    ...
    QuizTab:CreateButton({
        ...
        Callback = function()
            ...
            Rayfield:Notify({ Title = "Quiz Starting!", Content = q.name .. " is starting...", Duration = 3 })
...
QuizListTab:CreateLabel(quizListText)
...
SecretTab:CreateButton({
    ...
    Callback = function()
        ...
        if humanoid then
            humanoid:SetStateEnabled(Enum.HumanoidStateType.Flying, not humanoid:GetStateEnabled(Enum.HumanoidStateType.Flying))
            Rayfield:Notify({
                Title = "Flight Toggled",
                Content = "Flying has been " .. (humanoid:GetStateEnabled(Enum.HumanoidStateType.Flying) and "enabled" or "disabled"),
                Duration = 3,
            })
        else
            Rayfield:Notify({
                Title = "Error",
                Content = "Could not find Humanoid in your character.",
                Duration = 3,
            })
        end
    end
...
local quizNameInput = QuizMakerTab:CreateTextbox({
    Name = "Quiz Name",
    PlaceholderText = "Enter quiz name...",
    CurrentValue = "",
})

local questionInput = QuizMakerTab:CreateTextbox({
    Name = "Question",
    PlaceholderText = "Enter your question...",
    CurrentValue = "",
})

local optionAInput = QuizMakerTab:CreateTextbox({
    Name = "Option A",
    PlaceholderText = "Enter option A...",
    CurrentValue = "",
})

local optionBInput = QuizMakerTab:CreateTextbox({
    Name = "Option B",
    PlaceholderText = "Enter option B...",
    CurrentValue = "",
})

local optionCInput = QuizMakerTab:CreateTextbox({
    Name = "Option C",
    PlaceholderText = "Enter option C...",
    CurrentValue = "",
})

local optionDInput = QuizMakerTab:CreateTextbox({
    Name = "Option D",
    PlaceholderText = "Enter option D...",
    CurrentValue = "",
})
...
QuizMakerTab:CreateButton({
    ...
    Callback = function()
        ...
        table.insert(Quizzes, newQuiz)
        
        Rayfield:Notify({
            Title = "Quiz Created!",
            Content = "\"" .. quizName .. "\" has been added to the quiz list!",
            Duration = 3,
        })
        ...
        optionAInput:SetValue("")
        optionBInput:SetValue("")
        optionCInput:SetValue("")
        optionDInput:SetValue("")
        correctAnswerDropdown:SetCurrentOption({"A"})
        
        -- Update Quiz List tab
        local updatedListText = "Quiz List\n"
        for i, quiz in ipairs(Quizzes) do
            updatedListText = updatedListText .. "(" .. quiz.name .. ")\n"
        end
        updatedListText = updatedListText .. "1/" .. #Quizzes
        QuizListTab:UpdateLabel(updatedListText)
    end
})
...
VersionTab:CreateLabel("Quiz System v1.0")
...
VersionTab:CreateLabel("- Custom quiz creation")
